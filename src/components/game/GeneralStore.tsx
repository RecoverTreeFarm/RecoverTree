...
/* ---------------------------------------------------------------------------
 * The General Store 🏪 — a cozy interior location reached from the map.
 * The counter, register, shelves, and floor are drawn in CSS in the app's
 * pixel style (TODO(store-art): slice real furniture from
 * CozySpriteBundle/interior/ once coordinates are mapped); the shopkeeper is
 * a farmer sprite. Tapping the register OR the shopkeeper (both wear the
 * bouncing "!") opens the purchase menu: a 3x3 shelf grid + today's SALE
 * shelf. Purchases spend Coins server-side — never Fruits.
 * ------------------------------------------------------------------------- */
...
export function StoreScene({
  ...
  return (
    <div>
      ...
      >
        {/* wall */}
        <div className="absolute inset-x-0 top-0" style={{ height: "46%", background: "#c9a889" }} />
        <div
          className="absolute inset-x-0"
          style={{ top: "44%", height: 6, background: "var(--rf-soil-dark)", borderTop: "2px solid var(--rf-ink)" }}
        />
        {/* wooden floor */}
        <div
        ...
        <div
          className="absolute left-1/2 top-2 -translate-x-1/2 rounded border-2 border-[var(--rf-ink)] bg-[var(--rf-wood)] px-2 py-0.5 text-[10px] font-extrabold uppercase tracking-wide text-[var(--rf-cream)]"
        ...
        <span
          ...
          style={{ left: "8%", top: "12%", width: 52, height: 40, background: "var(--rf-sky)", border: "3px solid var(--rf-ink)", boxShadow: "inset 0 0 0 2px var(--rf-cream)" }}
        />
        <WallShelf items="🫙 🍯 🧺" style={{ right: "8%", top: "12%" }} />
        <WallShelf items="🌰 💧 ✨" style={{ right: "30%", top: "20%" }} />
        <WallShelf items="🪴 📦 🧴" style={{ left: "22%", top: "22%" }} />

        {/* floor decor */}
        <span aria-hidden className="absolute text-xl" style={{ left: "6%", bottom: "10%" }}>🪴</span>
        <span aria-hidden className="absolute text-lg" style={{ right: "6%", bottom: "8%" }}>📦</span>
        <span aria-hidden className="absolute text-lg" style={{ right: "12%", bottom: "16%" }}>🧺</span>
        {/* rug */}
        <span
          aria-hidden
          className="absolute left-1/2 -translate-x-1/2 rounded-[50%]"
          style={{ bottom: "6%", width: 150, height: 34, background: "var(--rf-red)", opacity: 0.55, border: "2px solid var(--rf-ink)" }}
        />

        {/* the counter */}
        <div
          className="absolute left-1/2 -translate-x-1/2"
          style={{ bottom: "30%", width: "56%", zIndex: 10 }}
        >
          <div
            className="relative"
            style={{
          ...
          >
            {/* the register sits on the counter — tap to shop */}
            <button
              type="button"
              aria-label="Cash register — open the store menu"
              title="Welcome to the General Store"
              onClick={() => setMenuOpen(true)}
              className="absolute -top-8 right-6 border-0 bg-transparent p-0"
            >
              <Bang />
        ...
        {/* the shopkeeper behind the counter — also opens the menu */}
        <button
          type="button"
          ...
          style={{ left: "38%", bottom: "42%", zIndex: 5 }}
        >
          <Bang />
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[9px] font-extrabold uppercase leading-tight text-center">
                    {cell.key === "water" ? `${state.water_amount} ${item.name}` : item.name}
                  </span>
                  <span className="text-[10px] font-bold">🪙 {state.prices[cell.key]}</span>
                  {disabled && gooseNote && (
                    <span className="absolute inset-x-0 -bottom-0.5 text-center text-[7px] font-bold uppercase text-[var(--rf-ink-soft)]">
                      {gooseNote}
                    </span>
                  )}
                </button>
